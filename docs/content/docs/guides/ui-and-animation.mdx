...
- **What tests look for** — the id, label or role of each element a test will find, written into
  the criteria so the tester doesn't have to guess a selector.
...
The normal [build loop](/docs/concepts/the-build-loop) runs after that, with two UI-specific rules:
