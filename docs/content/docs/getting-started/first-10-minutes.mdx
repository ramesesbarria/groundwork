...
description: "Here's what a first session looks like, from an idea to a committed first card. The example: a page where a book club votes on next month's book."
---

Want the longer version? The [recorded walkthrough](/docs/guides/walkthrough) follows a real build
chapter by chapter.
...
card instead. [Right-sizing →](/docs/concepts/right-sizing)
