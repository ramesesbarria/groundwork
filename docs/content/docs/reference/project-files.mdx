---
title: "Project files"
description: "Everything Groundwork knows lives in plain files in your repo, so you, your agent and your version control all see the same thing."
---
...
| `config.json` | The settings described in [Configuration](/docs/reference/configuration) | `gw-setup`, `gw-decide`, you |
...
- **Claude Code**: `.claude/skills/gw-*/`, `.claude/agents/gw-*.md`, `.claude/settings.json`, and a
  one-line `CLAUDE.md` that loads `AGENTS.md` and lists the commands. See [Claude Code](/docs/adapters/claude-code).
- **OpenCode**: `.opencode/commands/`, `.opencode/agents/`, `.opencode/plugins/`. See
  [OpenCode](/docs/adapters/opencode).
...
<Callout type="info" title="Evidence images are binary">
