...
[onboarding](/docs/guides/existing-projects). It ends by suggesting you commit the setup.
...
Turns the confirmed spec into phases and cards. Stack choices become [decision records](/docs/guides/decisions)
...
Runs the [build loop](/docs/concepts/the-build-loop) on the next ready card: tester → implementer →
...
The [light path](/docs/concepts/right-sizing) for small, low-risk changes: one pass, no card. Bugs get
...
[UI and animation →](/docs/guides/ui-and-animation)
...
[lessons ladder](/docs/concepts/lessons-ledger) — note → rule → guard — plus archiving stale ones.
