...
describe("groundwork CLI (smoke)", () => {
  it("prints the package version with --version", async () => {
    expect(await run(["--version"])).toEqual({ code: 0, output: pkg.version });
  });

  it("prints usage listing the planned commands when run with no arguments", async () => {
  ...
  it("fails with a helpful message on an unknown command", async () => {
